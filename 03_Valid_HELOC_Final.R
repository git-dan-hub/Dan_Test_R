...
#valid.start.date <- 2012 #Performance Year #For Trended Plots
...
model.data.years.valid <- model.data.years[model.data.years >= valid.start.date]
...
pred<-ROCR::prediction(valid.df$PD,valid.df$DefaultFlag)
...
#PD_PROD
pred<-ROCR::prediction(valid.df$PD_PROD,valid.df$DefaultFlag)
perf <- ROCR::performance(pred,"tpr","fpr")
max(attr(perf,'y.values')[[1]]-attr(perf,'x.values')[[1]])
...
prediction.data[['HELOC_NEW: FICO + BKRPT + Loan Age']] <- prediction.data.old ###### PTOM - CHANGES CHART TITLES #########
...
prediction.data <- list()
prediction.data[['HELOC_OLD (Production): FICO + BKRPT']] <- prediction.data.old ###### PTOM - CHANGES CHART TITLES #########
